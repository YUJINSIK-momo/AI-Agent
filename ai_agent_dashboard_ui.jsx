...
import { Search, Send, Sparkles, Github, Zap, Users, Code2, Bug, BarChart3, Megaphone, Headphones, Settings2, Crown, Boxes, Clock3 } from "lucide-react";
...
const agents = [
  ...
  { id: "cto", name: "CTO", kr: "기술 총괄", icon: Github, desc: "GitHub 코드 분석 / 리뷰", active: true },
...
const sampleReplies = {
  ...
  cto: "GitHub API 연결 후 Repository 구조, 최근 Commit, 변경 파일, 위험 코드를 분석할 수 있습니다.",
...
export default function AIAgentDashboard() {
  const [selected, setSelected] = useState("cto");
  const [input, setInput] = useState("");
  const [messages, setMessages] = useState([
    { role: "agent", agent: "cto", text: "안녕하세요. GitHub Repository 분석을 준비했습니다." },
  ]);
  ...
  const sendMessage = () => {
    ...
    setMessages((prev) => [
      ...prev,
      { role: "user", agent: selected, text },
      { role: "agent", agent: selected, text: sampleReplies[selected] },
    ]);
    setInput("");
  };
      ...
      <div className="grid min-h-screen grid-cols-1 lg:grid-cols-[390px_1fr_360px]">
        <aside className="border-r border-white/10 bg-[#0c0d14] p-5">
          ...
          <div className="mb-5 grid grid-cols-3 gap-3">
            ...
            <StatusBox label="TIME" value="10:17" />
            <StatusBox label="OUTPUT" value="0" />
          </div>

          <div className="relative mb-5">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30" size={17} />
            <Input placeholder="Agent 검색" className="border-white/10 bg-white/5 pl-10 text-white placeholder:text-white/30" />
          </div>
        ...
        <main className="relative flex min-h-screen flex-col bg-[radial-gradient(circle_at_center,rgba(245,158,11,0.08),transparent_35%),#080910]">
                    ...
                    <div key={idx} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                      <div
                        className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                          msg.role === "user" ? "bg-amber-500 text-black" : "bg-white/8 text-white/80"
                        }`}
                      >
                        {msg.text}
                      </div>
          ...
          <footer className="border-t border-white/10 p-5">
            <div className="mx-auto flex max-w-3xl gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-3">
              ...
              />
              <Button onClick={sendMessage} className="rounded-xl bg-amber-500 text-black hover:bg-amber-400">
                <Send size={17} />
              </Button>
            </div>
          </footer>
        ...
        <aside className="hidden border-l border-white/10 bg-[#101116] p-5 lg:block">
          ...
          <div className="space-y-3">
            ...
            <ReportItem title="상태" value="대기 중" />
            <ReportItem title="연동" value={selected === "cto" ? "GitHub API 가능" : "내부 Agent"} />
            <ReportItem title="권한" value="사용자 승인 필요" />
          </div>
